...
std::string CBoard::getBoard()
{
    ...
    std::ostringstream os;
        ...
        os << std::endl;
        os << "Material : " << m_material << std::endl;
        if (isKingInCheck())
        {
            os << "You are in check!" << std::endl;
        }
...
void CBoard::getBoardArray(int (&chessBoard) [64])
{
    for (int row=8; row>=1; --row)
    {
        for (int col=1; col<=8; ++col)
        {
            int number = (row+1)*10+col;
            int piece = m_board[number];
            chessBoard[((8-row))*8 + (col-1)] = piece;
        }
    }
}
